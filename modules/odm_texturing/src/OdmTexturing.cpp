...
struct lessThanKey
{
	inline bool operator() (const pcl::texture_mapping::Camera &cam1, const pcl::texture_mapping::Camera &cam2 ){
		return (cam1.texture_file < cam2.texture_file);
	}
};
...
void OdmTexturing::triangleToImageAssignment()
{
	...
	//Swap test for sanity check
	//    std::swap(*(cameras_.begin()), *(cameras_.end()-1));
	//    std::swap(*(cameras_.begin()+1), *(cameras_.end()-2));
	//    std::swap(*(cameras_.begin()+2), *(cameras_.end()-3));
	//    std::swap(*(cameras_.begin()+3), *(cameras_.end()-4));
	std::sort(cameras_.begin(), cameras_.end(), lessThanKey());
